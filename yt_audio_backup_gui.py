...
def _dedup_artist_in_filenames(files, log):
    ...
    out = []
    pat = re.compile(
        r'^(?:(?P<num>\d+)\s*-\s*)?(?P<a>[^-]+?)\s-\s(?P=a)\s-\s(?P<rest>.+)$', re.IGNORECASE
    )
    for p in files:
        try:
            m = pat.match(p.stem)
...
def join_via_wav_then_lame(
    ...
    try:
        ...
        rc = run_quiet(
            ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(listfile), str(concat_wav)]
        )
        if rc != 0:
...
def _find_tool_path(cmd: str) -> str | None:
    ...
    if os.name == "nt":
        ...
        candidates.append(
            os.path.join(
                os.environ.get("ProgramData", "C:\\ProgramData"), "chocolatey", "bin", f"{cmd}.exe"
            )
        )
        # Scoop shims
        if userprofile:
...
class App(tk.Tk):
    ...
    def _run(self) -> None:
        ...
        opts = ProcessingOptions(
            ...
            sanitize_filenames=bool(self.sanitize_var.get())
            if hasattr(self, "sanitize_var")
            else True,
            dedup_artist=bool(self.dedup_artist_var.get())
            if hasattr(self, "dedup_artist_var")
            else False,
            embed_metadata=bool(self.embed_metadata_var.get())
            if hasattr(self, "embed_metadata_var")
            else True,
            ...
            if hasattr(self, "vlc_segments_var")
            else False,
            random_join=bool(self.random_join_var.get())
            if hasattr(self, "random_join_var")
            else False,
            keep_temp_wavs=bool(self.keep_temp_wavs_var.get())
            if hasattr(self, "keep_temp_wavs_var")
            else False,
            sleep_between=int(self.sleep_between_var.get())
            if hasattr(self, "sleep_between_var")
            else 0,
            verbose_ydl=bool(self.verbose_ydl_var.get())
            if hasattr(self, "verbose_ydl_var")
            else False,
            cookies_file=Path(self.cookies_path_var.get())
            if hasattr(self, "cookies_path_var") and self.cookies_path_var.get()
            else None,
            cookies_browser=str(self.cookies_browser_var.get())
            if hasattr(self, "cookies_browser_var")
            else None,
            playlist_format=str(self.playlist_format_var.get())
            if hasattr(self, "playlist_format_var")
            else None,
        )

        t = threading.Thread(
            target=run_processing_task,
            args=(opts, self._log_q, self._progress_q, CANCEL_EVENT),
            daemon=True,
        )
        t.start()
    ...
    def _save_config(self) -> None:
        ...
        def _check_install_deps(self) -> None:
            ...
            if not missing:
                messagebox.showinfo("System Deps", "All tools already present.")
                return
            ...
            def add(cmd):
                cmds.append(cmd)
                self.log("Install cmd: " + " ".join(cmd))

            if "windows" in sysname:
                id_map = {
                    "yt-dlp": ["yt-dlp.yt-dlp"],
                    "ffmpeg": ["Gyan.FFmpeg"],
                    "mp3gain": ["GlenSawyer.MP3Gain"],
                }
                for tool in missing:
                    if tool == "ffprobe":
                        continue
                ...
                if "ffmpeg" in missing or "ffprobe" in missing:
                    pkgs.append("ffmpeg")
                if "mp3gain" in missing:
                    pkgs.append("mp3gain")
                if pkgs:
                    add(["brew", "install", *pkgs])
            else:
                if _sh.which("apt-get"):
                    ...
                    if "mp3gain" in missing:
                        pkgs.append("mp3gain")
                    if pkgs:
                        add(["sudo", "apt-get", "install", "-y", *pkgs])
                elif _sh.which("dnf"):
                    pkgs = []
                    if "yt-dlp" in missing:
                        pkgs.append("yt-dlp")
                    ...
                    if "mp3gain" in missing:
                        pkgs.append("mp3gain")
                    if pkgs:
                        add(["sudo", "pacman", "-S", "--noconfirm", *pkgs])
                ...
                try:
                    ...
                    for cmd in cmds:
                        try:
                            ...
                            rc = p.wait()
                            self.log(f"Command exited with code {rc}")
                        except Exception as e:
            ...
            if cmds:
                ...
                messagebox.showinfo(
                    "System Deps", "No installer commands available for this platform."
                )
